...
all:	libopencm3 libwwg
	...
	$(MAKE) -$(MAKEFLAGS) -C ./rtos_2 

clean:	clean_libopencm3
	for d in $(PROJECTS) ; do \
		$(MAKE) -C $$d clean ; \
	done
	$(MAKE) -$(MAKEFLAGS) -C ./rtos_2 clean
	$(MAKE) -$(MAKEFLAGS) -C ./rtos_2/libwwg clean

clobber: clobber_libopencm3
	for d in $(PROJECTS) ; do \
		$(MAKE) -C $$d clobber ; \
	done
	$(MAKE) -$(MAKEFLAGS) -C ./rtos_2 clobber
	$(MAKE) -$(MAKEFLAGS) -C ./rtos_2/libwwg clobber
